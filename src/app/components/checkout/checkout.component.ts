...
export class CheckoutComponent implements OnInit {
  ...
  getAllItem() {
    ...
    this.cartService.getCart(email).subscribe(data => {
      ...
      this.cartService.getAllDetail(this.cart.cartId).subscribe(data => {
        ...
        this.amountPaypal = (this.amount/22968);
      });
    });
  }
      ...
      Swal.fire({
        ...
        this.cartService.getCart(email).subscribe(data => {
          ...
          this.cartService.updateCart(email, this.cart).subscribe(data => {
            this.cart = data as Cart;
            this.paymentId = 1;
            this.orderService.post(email,this.paymentId ,this.cart).subscribe(data => {
  ...
  sendMessage(id:number) {
    ...
    this.notificationService.post(new Notification(0, this.cart.user.name + ' đã đặt 1 đơn hàng')).subscribe(data => {
