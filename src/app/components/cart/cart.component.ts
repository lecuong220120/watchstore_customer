...
export class CartComponent implements OnInit {
  ...
  update(id: number, quantity: number) {
    if (quantity < 1) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          this.cartService.getOneDetail(id).subscribe(data=>{
            ...
            this.cartDetail.price = (this.cartDetail.product.price * (1 - this.cartDetail.product.discount / 100)) * 1;
      ...
      this.cartService.getOneDetail(id).subscribe(data => {
        this.cartDetail = data as CartDetail;
        this.productService.getOne(this.cartDetail.product.productId).subscribe(data2=>{
          this.product = data2 as Product;
          if(this.product.quantity < quantity){
            this.toastr.error('Số lượng sản phẩm không đủ. Còn dư '+ this.product.quantity +' sản phẩm' ,'Hệ thống')
            this.cartDetail.quantity = this.product.quantity;
            this.cartDetail.price = (this.cartDetail.product.price * (1 - this.cartDetail.product.discount / 100)) * this.product.quantity;
            this.cartService.updateDetail(this.cartDetail).subscribe(data => {
              this.ngOnInit();
            }, error => {
              this.toastr.error('Lỗi!' + error.status, 'Hệ thống');
  ...
  quantityMinus(id: any, quantity: any) {
    --quantity;
    this.update(id, quantity);
  }

  quantityPlus(id: any, quantity: any) {
    ++quantity;
    this.update(id,quantity);
  }
}
